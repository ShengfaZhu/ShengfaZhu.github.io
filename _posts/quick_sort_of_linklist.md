...
<!-- TOC -->autoauto- [1. 位操作](#1-位操作)auto- [2. 位操作的基本用法](#2-位操作的基本用法)auto    - [2.1. 求1/2](#21-求12)auto    - [2.2. 判断奇偶](#22-判断奇偶)auto    - [2.3. 交换两个整数](#23-交换两个整数)auto    - [2.4. 变换正负号](#24-变换正负号)auto- [3. 位操作的进阶应用](#3-位操作的进阶应用)auto    - [3.1. 二进制中1的个数](#31-二进制中1的个数)auto    - [3.2 判断是否为2的幂](#32-判断是否为2的幂)auto    - [3.3. 只出现一次的数字](#33-只出现一次的数字)auto        - [3.3.1. 一个数字出现一次，其余数字出现两次](#331-一个数字出现一次其余数字出现两次)auto        - [3.3.2. 两个数字出现一次，其余数字出现两次](#332-两个数字出现一次其余数字出现两次)auto        - [3.3.3. 一个数字出现一次，其余数字出现3次](#333-一个数字出现一次其余数字出现3次)autoauto<!-- /TOC -->
